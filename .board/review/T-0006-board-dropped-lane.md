...
evidence:
  ...
  - criterion: "`npm run check:board` fails for a task in `dropped/` without a non-empty `## Dropped` section and passes once one is added, shown by a single evidence command that creates the task file, runs the check twice, and deletes the file"
    ...
    result: "First run: `.board/dropped/T-9001-x.md: dropped/ tasks need a non-empty ## Dropped section saying why`, exit 1 (the heading is present but empty). Second run, after adding a reason: `Board OK: 11 task(s).`, exit 0. File removed."
  ...
  - criterion: "`.board/README.md` documents two evidence conventions: for a conditional criterion that didn't apply, `command` is the check that showed it and `result` starts with `not applicable:` and says why; for a criterion met by the user's decision or judgment, `command` says where the decision is recorded (for example a PR comment or conversation) and `result` quotes it"
    ...
    result: "The task-file section documents both: a conditional criterion that didn't apply has `command` be the check that showed the condition didn't hold and `result` start with `not applicable:` and say why; a criterion met by the user's decision or judgment has `command` say where the decision is recorded (a PR comment, an issue, or the conversation) and `result` quote it."
...
Done: `scripts/check-board.ts` knows the `dropped/` lane. A dropped task needs a non-empty `## Dropped` section
(the text under that heading up to the next `##` heading); it needs no evidence or acceptance criteria, may have
`claimed_by` set or null, and is rejected if it has `verified_by` (the existing "only done/ tasks" rule). A task in
`ready/`, `active/`, `blocked/`, or `review/` that depends on a dropped task fails with a message saying to move it to
`backlog/` or `dropped/`; `backlog/`, `done/`, and `dropped/` tasks may depend on dropped tasks. `.board/README.md`
gains the lane row, a **Dropped** rule, and the two evidence conventions. `.board/dropped/.gitkeep` added.

Found: the sandbox's worktree guard refuses inline `printf -- '---...'` commands, so the evidence commands were run
from script files holding exactly the recorded text, from the worktree root. A verifier can paste them as-is.

Needs attention: the `## Dropped` check is a line scan, so a level-2 heading inside a fenced code block would end the
section early. No current task does that.
